...
> **说明**：该文档由我与 DeepSeek pro V4 连续对话20小时的内容总结而来，比较菜，烧了2亿 Token，只能保证文档中的所有代码经本人实测或 DeepSeek 反复搜索开源社区核实，不代表唯一解，请酌情使用。我是网络小白，完成此次实战前我只认得 IP、子网掩码、MAC、DNS，内容准确性和真实性本人没有能力核实，文档本身也是 DeepSeek 提炼生成的，我只做了文本语义提炼优化，如有错误之处或新的点子请提出，以推进开源精神的细水长流。
...
**三个原因**：
1. **pbr 反复失败**：pbr 启动会先执行 `Resetting routing`（重置整张路由表）再装 nft 规则，在 VPN/多 WAN 复杂环境极易失败 → 路由表被清空 → 全网断（社区同款失败案例多，本质是"启动即动全局路由"的设计缺陷）
2. **AGH 不够极致**：所有 DNS 模式都做不到"国内域名绝不发给 Mullvad"的域名级精确分流
3. **反向分流是社区公认方案**：基于 dnsmasq 官方标准功能 `server=/域名/上游`，有官方文档（man page），使用人数多，出问题容易搜到现成解法
...
**结论**：AGH 不是不能用，是不够好用。它的短板是国外 DNS 解析速度，把内外网拖成同一水平。对外网需求不高的人，AGH/全隧道是给自己找罪受，不如需要时手动开 VPN 客户端。
...
1. 分层定位：`ping 223.5.5.5`（路由层）vs `nslookup 域名`（DNS 层）
2. 对照实验：本机 vs 电脑（转发差异）；shell vs 进程（沙箱差异）；直连 vs 隧道（路径差异）
3. 抓包：`tcpdump -i any -nn "host 100.64.0.23" -c 20`
4. 查实际配置：`/var/etc/dnsmasq.conf.cfg01411c`（uci 值 ≠ 实际加载）
5. 语法检查：`dnsmasq --test`、`sh -x 脚本`
6. netns 对比：`ls -l /proc/<pid>/ns/` vs `readlink /proc/self/ns/net`
...
> **实战验证**：GL.iNet MT6000 Flint2 / OpenWrt 25.12.5 / 2026-08-12
